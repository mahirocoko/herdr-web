...
```text
server/
  index.ts              HTTP, WebSocket (/api/events, /api/terminal, /api/terminal/control), push routes, and static-file owner
  ...
  security.ts           origin, pane, action, and viewport validation
...
app/
  root.tsx              root document layout, head metadata, SW listener, and outlet context
...
src/
  ...
  components/           product surfaces (Panel, Question, History, Stream/Control, SettingsView, PaneDrawer)
  hooks/                useSnapshot, usePushSubscription, useTabNotificationPolicy, useTerminalStream, useTerminalControl, read
  services/             browser-to-server API client (api-client, push-client with tab-policy)
  types/                browser contracts
  utils/                event-stream parser, push helpers, pure selection, and geometry helpers
build/
